import { Events, CommandInteraction, EmbedBuilder } from 'discord.js';
import { PlaypulseBot } from '..';
import { BotCommand } from '../types';

module.exports = {
  name: Events.InteractionCreate,
  async execute(interaction: CommandInteraction, bot: PlaypulseBot) {
    ...
    const command = client.commands.get(interaction.commandName) as BotCommand;
          ...
          if (debugChannel && debugChannel.isTextBased()) {
            const debugEmbed = new EmbedBuilder()
              ...
              .addFields(
                ...
                { name: 'Guild', value: interaction.guildId || 'DM', inline: true }
              )
              .setColor(0xFF0000)
              .setTimestamp();
...
async function isAdmin(interaction: CommandInteraction): Promise<boolean> {
...
async function isPremiumUser(interaction: CommandInteraction): Promise<boolean> {
  if (!interaction.guild || !interaction.member) return false;
