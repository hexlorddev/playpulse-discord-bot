...
const createServerCommand = {
    ...
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .setDescription('Create a new server on Playpulse Hosting')
        .addStringOption(option => option.setName('plan')
        .setDescription('Server plan to use')
        .setRequired(true)
        .addChoices({ name: '💧 Basic - 2GB RAM', value: 'basic' }, { name: '💎 Premium - 4GB RAM', value: 'premium' }, { name: '🚀 Enterprise - 8GB RAM', value: 'enterprise' }, { name: '⚡ Custom - Contact Support', value: 'custom' }))
        .addStringOption(option => option.setName('region')
        .setDescription('Server region')
        .setRequired(true)
        .addChoices({ name: '🇺🇸 US East (Virginia)', value: 'us-east' }, { name: '🇺🇸 US West (California)', value: 'us-west' }, { name: '🇪🇺 EU Central (Germany)', value: 'eu-central' }, { name: '🌏 Asia Pacific (Singapore)', value: 'asia-pacific' }))
        .addStringOption(option => option.setName('type')
        .setDescription('Server type')
        .setRequired(false)
        .addChoices({ name: '⛏️ Minecraft Server', value: 'minecraft' }, { name: '🤖 Discord Bot', value: 'discord-bot' }, { name: '🌐 Web Application', value: 'web-app' }, { name: '🎮 Game Server', value: 'game-server' }))
        .addStringOption(option => option.setName('name')
        .setDescription('Server name (optional)')
        .setRequired(false))
        .addBooleanOption(option => option.setName('auto-start')
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                .addFields({ name: '📋 Server Name', value: serverName, inline: true }, { name: '💽 Plan', value: getPlanDisplayName(plan), inline: true }, { name: '🌍 Region', value: getRegionDisplayName(region), inline: true }, { name: '🆔 Server ID', value: serverData.data.id, inline: true }, { name: '📡 Status', value: '🟡 Starting...', inline: true }, { name: '🎮 Type', value: getServerTypeDisplayName(serverType), inline: true })
                .setColor(0x00FF00)
            ...
            setTimeout(async () => {
                const followUpEmbed = new discord_js_1.EmbedBuilder()
                    ...
                    .addFields({ name: '📊 Check Status', value: '`/server-status ' + serverData.data.id + '`', inline: true }, { name: '📋 View Logs', value: '`/view-logs ' + serverData.data.id + '`', inline: true }, { name: '🔄 Reboot', value: '`/reboot-server ' + serverData.data.id + '`', inline: true }, { name: '⚙️ Manage', value: `[Control Panel](${process.env.PANEL_API_URL}/servers/${serverData.data.id})`, inline: false })
                    .setColor(0x0099FF)
...
function getMaxServersForUser(userId) {
    // This would check user's subscription tier
    // For now, return default limit
    return 3; // Free tier limit
}
async function createServerViaAPI(userId, options) {
    try {
        const response = await axios_1.default.post(`${process.env.PANEL_API_URL}/servers/create`, {
            userId,
            ...options
        }, {
            headers: {
...
function getServerTypeDisplayName(type) {
    const types = {
